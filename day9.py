...
#  Getting extra memory for refrencing larger indicies
...
while i < len(puzzle_input):
    ...
    if opcode == 3:
        ...
        if mode == 0:
            ...
            puzzle_input[puzzle_input[i + 1] + relative_base] = ui
        
        i += 2

    elif opcode == 4: 
        if mode == 0:
            output = puzzle_input[puzzle_input[i + 1]]
        elif mode == 1:
            output = puzzle_input[i + 1]
        else:
            output = puzzle_input[puzzle_input[i + 1] + relative_base]
        print(f"{output}", end=",")
        i += 2
        ...
        if para1mode == 0:
            input1 = puzzle_input[para1]
        elif para1mode == 1:
            input1 = para1
        else:
            input1 = puzzle_input[para1 + relative_base]

        if para2mode == 0:
            input2 = puzzle_input[para2]
        elif para2mode == 1:
            input2 = para2
        else:
            input2 = puzzle_input[para2 + relative_base]
        ...
        if para3mode == 0:
            ...
            puzzle_input[para3 + relative_base] = output
        i += 4
    ...
    # --- PART TWO ---
        ...
        para1mode = mode
        if para1mode == 0:
            para1 = puzzle_input[puzzle_input[i + 1]]
        elif para1mode == 1:
            para1 = puzzle_input[i + 1]
        else:
            para1 = puzzle_input[puzzle_input[i + 1] + relative_base]

        para2mode = inst[3] if len(inst) > 3 else 0
        if para2mode == 0:
            para2 = puzzle_input[puzzle_input[i + 2]]
        elif para2mode == 1:
            para2 = puzzle_input[i + 2]
        else:
            para2 = puzzle_input[puzzle_input[i + 2] + relative_base]
        ...
        para3 = puzzle_input[i + 3] if para3mode == 0 else (puzzle_input[i + 3] + relative_base)
...
# -------------- PART TWO --------------
# Just enter 2 instead of 1 for the only program input